...
import type { ImportFile, User } from "@/payload-types";

/** The file buffer and its associated metadata. */
export interface ImportFileAttachment {
  /** Raw file content. */
  data: Buffer;
...
export interface CreateImportFileParams {
  /** Payload instance. */
  payload: Payload;
 ...
 */
export const createImportFileAndQueueDetection = async ({
  payload,
  importFileData,
  file,
  user,
}: CreateImportFileParams): Promise<CreateImportFileResult> => {
  ...
  return { importFileId: importFile.id };
};
