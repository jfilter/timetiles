...
export const ingestJobFields: Field[] = [
  // Basic Information
  {
    name: "ingestFile",
    type: "relationship",
    ...
    required: true,
    admin: { description: "Source file for this ingest job" },
  },
  {
    name: "dataset",
    type: "relationship",
  ...
  // Schema Version Reference
  {
    ...
    admin: {
      ...
      condition: (data) =>
        [PROCESSING_STAGE.GEOCODE_BATCH, PROCESSING_STAGE.CREATE_EVENTS, PROCESSING_STAGE.COMPLETED].includes(
          data.stage
        ),
    },
  },
  ...
  // Results
  {
    ...
    admin: {
      ...
      condition: (data) => data.stage === PROCESSING_STAGE.COMPLETED || data.stage === PROCESSING_STAGE.FAILED,
    },
  },
    ...
    hooks: {
      afterRead: [
        ({ data }) => {
          ...
          const fileName = typeof data?.ingestFile === "object" ? data.ingestFile.filename : "Unknown File";
