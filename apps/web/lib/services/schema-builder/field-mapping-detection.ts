...
export const detectFieldMappings = (fieldStats: Record<string, FieldStatistics>, language: string): FieldMappings => {
  // Use consolidated plugin implementation for geo field detection
  const geoResult = detectGeoFieldsFromPlugin(fieldStats);
  ...
  return {
    ...
    latitudePath: geoResult?.latitude?.path ?? null,
    longitudePath: geoResult?.longitude?.path ?? null,
    locationPath: geoResult?.locationField?.path ?? detectField(fieldStats, "location", language),
  };
};
