...
// Constants
const IMPORT_JOBS_COLLECTION = "ingest-jobs";
...
export class ErrorRecoveryService {
  ...
  private static async checkRetryQuota(
    ...
    const ingestFileId = requireRelationId(job.ingestFile, "ingestJob.ingestFile");
  ...
  static async recoverFailedJob(
    ...
    try {
      ...
      if (!job) {
        return { success: false, action: "job_not_found", error: "Ingest job not found" };
      }
  ...
  private static classifyError(job: IngestJob): ErrorClassification {
   ...
   * @private
   */
  private static determineRecoveryStage(job: IngestJob, classification: ErrorClassification): ProcessingStage {
  ...
  static async processPendingRetries(payload: Payload): Promise<void> {
    try {
      ...
      for (const job of readyJobs.docs) {
        logger.info("Processing scheduled retry", { ingestJobId: job.id });
        ...
        if (updateResult.docs.length > 0) {
          logger.info("Set recovery stage (job queued via hook)", { ingestJobId: job.id, stage: recoveryStage });
        }
      }
    } catch (error) {
  ...
  static async resetJobToStage(
    ...
    try {
      ...
      const updateData: Partial<IngestJob> = {
        stage: targetStage,
        lastRetryAt: new Date().toISOString(),
        errorLog: {
          ...getErrorLogState(job),
