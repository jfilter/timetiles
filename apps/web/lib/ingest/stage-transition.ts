...
export class StageTransitionService {
  ...
  static async processStageTransition(
    payload: Payload,
    job: IngestJob,
    previousJob: IngestJob | undefined
    ...
    if (previousJob?.stage && !this.validateStageTransition(previousJob.stage, job.stage)) {
      ...
      logger.error(error, { ingestJobId: jobId });
   ...
   */
  private static async queueStageJob(payload: Payload, job: IngestJob): Promise<{ queued: boolean; jobType?: string }> {
    const jobType = STAGE_TO_JOB_TYPE[job.stage];

    // Stages with no automatic job
    if (jobType === null || jobType === undefined) {
      if (job.stage === PROCESSING_STAGE.AWAIT_APPROVAL) {
        logger.info("Import requires manual approval", { ingestJobId: job.id });
      } else if (job.stage === PROCESSING_STAGE.COMPLETED) {
        logger.info("Ingest job completed successfully", { ingestJobId: job.id });
      } else if (job.stage === PROCESSING_STAGE.FAILED) {
        logger.error("Ingest job failed", { ingestJobId: job.id });
      } else {
        logger.warn("Unknown stage for job queuing", { ingestJobId: job.id, stage: job.stage });
      }
      return { queued: false };
    }
    ...
    const input =
      jobType === JOB_TYPES.GEOCODE_BATCH ? { ingestJobId: job.id, batchNumber: 0 } : { ingestJobId: job.id };
