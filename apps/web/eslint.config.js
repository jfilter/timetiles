 ...
 *
 * Extends the base Next.js configuration with custom rules for:
 * - Migration files (allowing longer functions for SQL/schema changes)
...
export default [
    ...
    rules: {
      "i18next/no-literal-string": [
        "warn",
        {
          ...
          "jsx-components": { exclude: ["style", "script", "Head", "Script"] },
          words: {
            exclude: [
              // Single characters, punctuation, CSS-like values
              "^[A-Z]$",
              "^\\d+$",
              "^[\\s·•→←↑↓|/\\-–—,.:;!?@#$%^&*()\\[\\]{}]+$",
              // Common technical patterns
              "^(px|em|rem|vh|vw|%|auto|none|inherit|flex|grid|block|inline)$",
              "^(GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS)$",
              "^(text|password|email|number|tel|url|search|date|time|file|hidden|submit|button|checkbox|radio)$",
              "^https?://",
              "^/",
            ],
          },
          ...
          "object-properties": { exclude: ["className", "style", "variant", "size", "type", "method", "key", "as"] },
        },
      ],
    },
  },
  ...
  // Seed files - relax rules for seed data generation
  {
    ...
    rules: {
      "sonarjs/max-lines": "off", // Seed files often contain extensive data definitions
