/**
 * Remove adaptive scaling from cluster_events(). The zoom-level-based
 * pixel_radius alone determines grid cell size — matching how Supercluster
 * and other standard clustering algorithms work. Adaptive scaling
 * over-consolidated clusters at low zoom levels (3 clusters for all of Berlin).
 *
 * Also tunes pixel_radius values to produce ~20-50 clusters per zoom level.
 *
 * @module
 */
...
// --- Fixed radius (no adaptive), with extent_degrees ---

const buildFixedRadiusClusterFunction = (dataColumn: string, fieldExpression: string, lngFilter: string) => `
  ...
  BEGIN
    -- Fixed pixel radius per zoom level (no adaptive scaling).
    -- Like Supercluster: zoom alone determines spatial resolution.
    pixel_radius := CASE
...
// --- Previous function with adaptive scaling (for down migration) ---

const buildAdaptiveClusterFunction = (dataColumn: string, fieldExpression: string, lngFilter: string) => `
  CREATE FUNCTION cluster_events(
    p_min_lng double precision,
    p_min_lat double precision,
    p_max_lng double precision,
...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.execute(
    sql`${sql.raw(buildFixedRadiusClusterFunction("transformed_data", TRANSFORMED_FIELD_EXPRESSION, lngFilter))}`,
  );
}
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  await db.execute(sql`${sql.raw(DROP_FUNCTION)}`);
  await db.execute(
    sql`${sql.raw(buildAdaptiveClusterFunction("transformed_data", TRANSFORMED_FIELD_EXPRESSION, lngFilter))}`,
  );
}
