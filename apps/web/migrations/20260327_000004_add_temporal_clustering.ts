/**
 * Add `cluster_events_temporal()` PostgreSQL function for beeswarm chart.
 *
 * Adaptive function: returns individual events when total <= threshold,
 * per-dataset-per-bucket clusters when total > threshold. Reuses the same
 * filter model as `calculate_event_histogram()`.
 *
 * @module
 */
...
/** Shared WHERE clause for the filter model (identical to calculate_event_histogram). */
...
${sharedFilterWhere()};
    ...
    IF v_total <= p_individual_threshold THEN
      ...
      SELECT
        ...
        e.dataset_id::integer,
        d.name::text AS dataset_name,
...
${sharedFilterWhere()}
    ...
    -- CLUSTERED MODE: compute bucket size then aggregate per dataset per bucket
...
${sharedFilterWhere()}
    ...
    SELECT
      ...
      fe.dataset_id::integer,
      fe.ds_name::text AS dataset_name,
    ...
    WHERE fe.id IS NOT NULL  -- Only return buckets that have events
    GROUP BY b.b_start, b.b_end, fe.dataset_id, fe.ds_name
    ORDER BY b.b_start, fe.dataset_id;
  END;
