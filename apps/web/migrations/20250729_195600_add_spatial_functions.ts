...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  // Create spatial GIST index for location data
  await db.execute(sql`
  ...
  // Create the clustering function for map clustering
  await db.execute(sql`
    ...
    BEGIN
      -- Zoom-based clustering distance with fine granularity for high zoom levels
      cluster_distance := CASE
        WHEN p_zoom >= 16 THEN 0.0001   -- ~11m (very small for individual events)
        WHEN p_zoom >= 15 THEN 0.0005   -- ~55m
        WHEN p_zoom >= 14 THEN 0.001    -- ~110m
        WHEN p_zoom >= 13 THEN 0.002    -- ~220m
        WHEN p_zoom >= 12 THEN 0.005    -- ~550m
        WHEN p_zoom >= 11 THEN 0.008    -- ~880m
        WHEN p_zoom >= 10 THEN 0.01     -- ~1.1km
        WHEN p_zoom >= 9 THEN 0.02      -- ~2.2km
        WHEN p_zoom >= 8 THEN 0.05      -- ~5.5km
        WHEN p_zoom >= 7 THEN 0.08      -- ~8.8km
        WHEN p_zoom >= 6 THEN 0.1       -- ~11km
        ELSE 0.2                         -- ~22km
      END;
  ...
  // Create the histogram function for time-based aggregation
  await db.execute(sql`
