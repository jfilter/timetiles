/**
 * Add custom PostgreSQL functions and GIN index for event queries.
 *
 * Creates the `cluster_events()` and `calculate_event_histogram()` functions
 * used by the geospatial and temporal API endpoints, plus a GIN index on the
 * `original_data` JSONB column for fast field-level queries.
 *
 * @module
 */
...
const TOP_LEVEL_FIELD_EXPRESSION = "e.original_data->>ff.field_key";

export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(
    sql`${sql.raw(buildClusterEventsFunction(NESTED_FIELD_EXPRESSION))}`,
  );
  await db.execute(
    sql`${sql.raw(buildHistogramFunction(NESTED_FIELD_EXPRESSION))}`,
  );
  await db.execute(sql`
    CREATE INDEX IF NOT EXISTS "events_original_data_gin_idx" ON "payload"."events" USING GIN ("original_data");
  `);
  await db.execute(sql`
    CREATE UNIQUE INDEX IF NOT EXISTS "datasets_catalog_name_unique_idx"
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  await db.execute(
    sql`${sql.raw(buildClusterEventsFunction(TOP_LEVEL_FIELD_EXPRESSION))}`,
  );
  await db.execute(
    sql`${sql.raw(buildHistogramFunction(TOP_LEVEL_FIELD_EXPRESSION))}`,
  );
  await db.execute(sql`
    DROP INDEX IF EXISTS "payload"."events_original_data_gin_idx";
  `);
  await db.execute(sql`
