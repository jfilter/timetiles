/**
 * Fix anti-meridian (dateline) crossing in cluster_events() and calculate_event_histogram().
 *
 * Both functions previously used `BETWEEN min_lng AND max_lng` for longitude filtering.
 * When a viewport crosses the dateline (e.g., west=170, east=-170), `BETWEEN 170 AND -170`
 * returns zero rows. The fix uses CASE WHEN to detect the crossing and applies an OR clause.
 *
 * @module
 */
...
/** Anti-meridian-safe longitude filter for direct parameter pairs (cluster_events). */
...
/** Anti-meridian-safe longitude filter for JSONB bounds (calculate_event_histogram). */
const buildBoundsLngFilter = (col: string) => `
          AND (p_filters->'bounds' IS NULL OR (
            CASE WHEN (p_filters->'bounds'->>'minLng')::double precision
              ...
              THEN ${col} BETWEEN (p_filters->'bounds'->>'minLng')::double precision
                AND (p_filters->'bounds'->>'maxLng')::double precision
              ELSE (${col} >= (p_filters->'bounds'->>'minLng')::double precision
                 OR ${col} <= (p_filters->'bounds'->>'maxLng')::double precision)
            END
            AND e.location_latitude BETWEEN (p_filters->'bounds'->>'minLat')::double precision
              AND (p_filters->'bounds'->>'maxLat')::double precision
          ))`;

/** Naive BETWEEN for longitude (used in down() to restore pre-fix state). */
const buildDirectLngBetween = (col: string, minVar: string, maxVar: string) =>
  `AND ${col} BETWEEN ${minVar} AND ${maxVar}`;

/** Naive BETWEEN for JSONB bounds longitude (used in down()). */
const buildBoundsLngBetween = (col: string) => `
          AND (p_filters->'bounds' IS NULL OR (
            ${col} BETWEEN (p_filters->'bounds'->>'minLng')::double precision
              AND (p_filters->'bounds'->>'maxLng')::double precision
            AND e.location_latitude BETWEEN (p_filters->'bounds'->>'minLat')::double precision
              AND (p_filters->'bounds'->>'maxLat')::double precision
          ))`;

// --- Function builders (parameterized by field expression + lng filter) ---

const buildClusterEventsFunction = (fieldExpression: string, lngFilter: string) => `
  ...
  BEGIN
    ...
    WITH filtered_events AS (
      SELECT
        ...
        d.catalog_id,
        e.event_timestamp,
        e.original_data->>'title' as title
...
const buildHistogramFunction = (fieldExpression: string, boundsFilter: string) => `
...
const NESTED_FIELD_EXPRESSION =
  "e.original_data #>> string_to_array(ff.field_key, '.')";

export async function up({ db }: MigrateUpArgs): Promise<void> {
  const clusterLngFilter = buildDirectLngFilter(
    "e.location_longitude",
    "p_min_lng",
    "p_max_lng",
  );
  const histogramBoundsFilter = buildBoundsLngFilter("e.location_longitude");

  await db.execute(
    sql`${sql.raw(buildClusterEventsFunction(NESTED_FIELD_EXPRESSION, clusterLngFilter))}`,
  );
  await db.execute(
    sql`${sql.raw(buildHistogramFunction(NESTED_FIELD_EXPRESSION, histogramBoundsFilter))}`,
  );
}

export async function down({ db }: MigrateDownArgs): Promise<void> {
  // Restore original naive BETWEEN longitude filters (preserving nested field expression)
  const clusterLngBetween = buildDirectLngBetween(
    "e.location_longitude",
    "p_min_lng",
    "p_max_lng",
  );
  const histogramLngBetween = buildBoundsLngBetween("e.location_longitude");

  await db.execute(
    sql`${sql.raw(buildClusterEventsFunction(NESTED_FIELD_EXPRESSION, clusterLngBetween))}`,
  );
  await db.execute(
    sql`${sql.raw(buildHistogramFunction(NESTED_FIELD_EXPRESSION, histogramLngBetween))}`,
  );
}
