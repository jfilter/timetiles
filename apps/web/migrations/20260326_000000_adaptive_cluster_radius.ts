/**
 * Make cluster_events() adaptive: scale pixel_radius based on event count
 * in the viewport. With 20k+ events the static radius produces hundreds of
 * overlapping clusters; this migration adds a logarithmic scale factor that
 * increases the grid cell size when many events are present.
 *
 * @module
 */
...
const buildAdaptiveClusterFunction = (dataColumn: string, fieldExpression: string, lngFilter: string) => `
  ...
  BEGIN
    -- Base pixel radius by zoom level
    pixel_radius := CASE
    ...
    -- Scale pixel_radius based on event count (log2-based, capped at 3x)
    -- Below 500 events: no scaling. Above: smooth logarithmic increase.
...
// --- Static (previous) cluster function for down migration ---

const buildStaticClusterFunction = (dataColumn: string, fieldExpression: string, lngFilter: string) => `
  CREATE OR REPLACE FUNCTION cluster_events(
    p_min_lng double precision,
    p_min_lat double precision,
    p_max_lng double precision,
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  await db.execute(
    sql`${sql.raw(buildStaticClusterFunction("transformed_data", TRANSFORMED_FIELD_EXPRESSION, lngFilter))}`,
  );
}
