/**
 * Further increase grid cell size at city zoom levels and steepen adaptive
 * scaling to produce fewer, more distinct clusters (target ~20-40 at zoom 12
 * for Berlin, down from ~165).
 *
 * @module
 */
...
// --- V2: larger grid cells at city zoom, steeper scaling ---

const buildClusterFunctionV2 = (dataColumn: string, fieldExpression: string, lngFilter: string) => `
  ...
  BEGIN
    -- Larger grid cells at city/district zoom for fewer, more distinct clusters
    pixel_radius := CASE
      ...
      WHEN p_zoom <= 11 THEN 65.0   -- City level (was 55)
      WHEN p_zoom <= 13 THEN 50.0   -- District level (was 45)
    ...
    -- Steeper adaptive scaling: coefficient 0.5, cap at 5x
...
// --- Previous V1 function for down migration ---

const buildClusterFunctionV1 = (dataColumn: string, fieldExpression: string, lngFilter: string) => `
  CREATE OR REPLACE FUNCTION cluster_events(
    p_min_lng double precision,
    p_min_lat double precision,
    p_max_lng double precision,
  ...
  BEGIN
    pixel_radius := CASE
      ...
      WHEN p_zoom <= 11 THEN 55.0
      WHEN p_zoom <= 13 THEN 45.0
      ELSE 35.0
    END;
...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.execute(
    sql`${sql.raw(buildClusterFunctionV2("transformed_data", TRANSFORMED_FIELD_EXPRESSION, lngFilter))}`,
  );
}
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  await db.execute(
    sql`${sql.raw(buildClusterFunctionV1("transformed_data", TRANSFORMED_FIELD_EXPRESSION, lngFilter))}`,
  );
}
