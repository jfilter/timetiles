...
describe.sequential('Events API Route', () => {
  ...
  beforeEach(async () => {
    ...
    if (remainingEvents.docs.length > 0) {
      console.warn(`Database not clean before test - found ${remainingEvents.totalDocs} events`);
      ...
      if (stillRemaining.docs.length > 0) {
        throw new Error(`Database cleanup failed - still found ${stillRemaining.totalDocs} events after TRUNCATE`);
      }
    }
  });
  ...
  const createTestEvent = async (data: any, location: any, dataset: any, eventTimestamp?: string | null) => {
    ...
    return await payload.create({
      ...
      data: {
        ...
        eventTimestamp: eventTimestamp === null ? undefined : (eventTimestamp || new Date().toISOString()),
        isValid: true,
        slug, // Provide explicit slug to avoid hook validation issues
      },
    });
  ...
  test('filters events by catalog', async () => {
    ...
    const url = new URL(`http://localhost:3000/api/events?catalog=${catalog1.slug}`);
  ...
  test('filters events by multiple datasets', async () => {
    ...
    const url = new URL(`http://localhost:3000/api/events?datasets=${dataset1.slug}&datasets=${dataset2.slug}`);
    const request = new NextRequest(url);
    const response = await GET(request);
    const data = await response.json();
  ...
  test('filters events by geographic bounds', async () => {
    ...
    const url = new URL(`http://localhost:3000/api/events?bounds=${encodeURIComponent(bounds)}`);
    const request = new NextRequest(url);
    const response = await GET(request);
    const data = await response.json();
  ...
  test('filters events by date range', async () => {
    ...
    const url = new URL('http://localhost:3000/api/events?startDate=2024-06-01&endDate=2024-06-30');
    const request = new NextRequest(url);
    const response = await GET(request);
    const data = await response.json();
  ...
  test('combines multiple filters correctly', async () => {
    ...
    const url = new URL(`http://localhost:3000/api/events?catalog=${catalog1.slug}&startDate=2024-06-01&endDate=2024-06-30&bounds=${encodeURIComponent(bounds)}`);
    const request = new NextRequest(url);
    const response = await GET(request);
    const data = await response.json();
