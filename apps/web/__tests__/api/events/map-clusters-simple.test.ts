...
describe("Map-clusters API test", () => {
  ...
  it("should handle map-clusters API request without errors", async () => {
    try {
      ...
      console.log("Payload db keys:", testEnv.payload.db ? Object.keys(testEnv.payload.db) : 'undefined');
      if (testEnv.payload.db) {
        ...
        if (testEnv.payload.db.adapter) {
          ...
          console.log("DB adapter keys:", Object.keys(testEnv.payload.db.adapter));
        }
      }
      ...
      url.searchParams.set("bounds", JSON.stringify({
        north: 40.7589,
        south: 40.7489,
        east: -73.9741,
        west: -73.9841
