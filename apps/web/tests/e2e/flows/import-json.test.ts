 ...
 * - Scheduled import JSON API configuration via REST API
 *
 * @module
 * @category E2E Tests
 */
...
test.describe("Import Wizard - JSON URL Input", () => {
  ...
  test("should show URL input tab and accept URL for fetching", async ({ page }) => {
    ...
    const apiResponsePromise = page.waitForResponse((resp) => resp.url().includes("/api/import/preview-schema/url"), {
      timeout: 30_000,
    });
...
// ---------------------------------------------------------------------------
// Test 3: Scheduled import JSON API config via REST API
// ---------------------------------------------------------------------------

test.describe("Scheduled Import - JSON API Configuration", () => {
  test.describe.configure({ mode: "serial", timeout: 60_000 });

  let token: string;
  let baseUrl: string;
  let scheduledImportId: number;
  ...
  test("should create catalog and dataset for scheduled import", async ({ request }) => {
    const uniqueId = Date.now();
  ...
  test("should create scheduled import with JSON API configuration", async ({ request }) => {
    const uniqueId = Date.now();
    ...
    const response = await request.post(`${baseUrl}/api/scheduled-imports`, {
      data: scheduledImportData,
      headers: { "Content-Type": "application/json", Authorization: `JWT ${token}` },
    });
  ...
  test("should retrieve scheduled import with JSON API config intact", async ({ request }) => {
    const response = await request.get(`${baseUrl}/api/scheduled-imports/${scheduledImportId}`, {
  ...
  test("should update scheduled import JSON API config", async ({ request }) => {
    const response = await request.patch(`${baseUrl}/api/scheduled-imports/${scheduledImportId}`, {
      data: {
        advancedOptions: {
          responseFormat: "json",
          jsonApiConfig: {
  ...
  test("should clean up test data", async ({ request }) => {
    ...
    if (scheduledImportId) {
      const deleteResponse = await request.delete(`${baseUrl}/api/scheduled-imports/${scheduledImportId}`, {
        headers: { Authorization: `JWT ${token}` },
      });
