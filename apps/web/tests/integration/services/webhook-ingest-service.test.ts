...
import { createIntegrationTestEnvironment, withScheduledIngest, withUsers } from "../../setup/integration/environment";
...
describe.sequential("Webhook Import Service Integration", () => {
  ...
  beforeEach(async () => {
    ...
    // Create fresh scheduled ingest using local test server
    const timestamp = Date.now();
  ...
  describe("Import File Creation", () => {
    it("should create import file from webhook trigger", async () => {
      ...
      expect(ingestFile.originalName).toContain("Webhook Import Test");
      ...
      expect(ingestFile.metadata).toMatchObject({
        scheduledExecution: { scheduledIngestId: testScheduledIngest.id, executionTime: expect.any(String) },
      });

      // Verify file was saved
      if (ingestFile.filename) {
        expect(ingestFile.filename).toMatch(/url-import-.*\.csv$/);
    ...
    it("should handle Excel files correctly", async () => {
      ...
      if (ingestFile.filename) {
        expect(ingestFile.filename).toMatch(/\.xlsx$/);
        expect(ingestFile.mimeType).toBe("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
      }
    });
  });
  ...
  describe("Duplicate Content Detection", () => {
    afterEach(async () => {
      ...
      for (const file of existingFiles.docs) {
        await payload.delete({ collection: "ingest-files", id: file.id });
      }
    });
    ...
    it("should detect and skip duplicate content", async () => {
      ...
      await payload.update({ collection: "ingest-files", id: firstFileId, data: { status: "completed" } });
      ...
      expect(result2.output.ingestFileId.toString()).toBe(firstFileId.toString());

      // Verify scheduled ingest was updated with duplicate info
  ...
  describe("Authentication Handling", () => {
    it("should pass bearer token authentication", async () => {
      // Update scheduled ingest with bearer auth to test endpoint
      await payload.update({
        collection: "scheduled-ingests",
        id: testScheduledIngest.id,
        data: {
  ...
  describe("Multi-Sheet Configuration", () => {
    it("should pass multi-sheet config to import file", async () => {
      ...
      await payload.update({ collection: "scheduled-ingests", id: testScheduledIngest.id, data: { multiSheetConfig } });
      ...
      if (
        typeof ingestFile.metadata !== "object" ||
        ingestFile.metadata === null ||
        Array.isArray(ingestFile.metadata)
      ) {
