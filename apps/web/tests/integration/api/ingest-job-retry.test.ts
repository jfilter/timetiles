...
describe.sequential("Import Job Retry API", () => {
  ...
  beforeEach(async () => {
    ...
    // Create test user first (catalog needs an owner for ingest-file validation)
...
Event 2,2024-01-02,Location 2`;
      ...
      const { ErrorRecoveryService } = await import("@/lib/ingest/error-recovery");
      ...
      const updatedJob = await payload.findByID({ collection: "ingest-jobs", id: failedJob.id });
...
Event,2024-01-01`;
      ...
      const resetJob = await payload.findByID({ collection: "ingest-jobs", id: failedJob.id });
...
Event 2,2024-01-02`;
      ...
      const { ingestFile: importFile1 } = await withIngestFile(testEnv, testCatalogId, csvContent1, {
      ...
      const { ingestFile: importFile2 } = await withIngestFile(testEnv, testCatalogId, csvContent2, {
