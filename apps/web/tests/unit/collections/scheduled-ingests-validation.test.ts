...
describe("scheduled-ingests validation", () => {
  describe("validateCronExpression", () => {
    it("rejects a range with multiple step separators", () => {
      expect(validateCronExpression("0 1-5/2/3 * * *")).not.toBe(true);
    });

    it("accepts named months and weekdays supported by the cron engine", () => {
      expect(validateCronExpression("0 0 * JAN MON")).toBe(true);
    });
    ...
    it("rejects partially numeric single cron values", () => {
      expect(validateCronExpression("5abc 0 * * *")).toMatch(/Cron expression/);
    });

    it("rejects partially numeric range cron values", () => {
      expect(validateCronExpression("0 1-5xyz * * *")).toMatch(/Cron expression/);
    });

    it("rejects partially numeric step cron values", () => {
      expect(validateCronExpression("*/2oops 0 * * *")).toMatch(/Cron expression/);
    });

    it("rejects partially numeric list cron values", () => {
      expect(validateCronExpression("0 1,2oops * * *")).toMatch(/Cron expression/);
    });
