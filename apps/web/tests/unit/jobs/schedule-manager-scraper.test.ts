...
describe.sequential("scheduleManagerJob — scraper scheduling", () => {
  ...
  it.each(["0 0 31 2 *", "invalid cron", "0 0 * * * *"])(
    "disables an unusable schedule without queueing: %s",
    async (schedule) => {
      const { mockPayload, mockJob, mockReq } = createMockContext();
      vi.setSystemTime(new Date("2026-03-15T12:05:00Z"));
      setupScrapersOnly(mockPayload, [{ id: 12, name: "Invalid date", enabled: true, schedule }]);
