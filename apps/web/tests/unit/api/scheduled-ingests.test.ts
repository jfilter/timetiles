/**
 * Unit tests for the scheduled-ingests trigger route.
 *
 * Tests the POST /api/scheduled-ingests/[id]/trigger apiRoute handler.
 *
 * @module
 * @category Tests
 */
...
const createRequest = () => new NextRequest("http://localhost/api/scheduled-ingests/1/trigger", { method: "POST" });

describe.sequential("POST /api/scheduled-ingests/[id]/trigger", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("should return 404 when schedule not found or access denied", async () => {
    ...
    const data = await response.json();
    expect(data.error).toBe("scheduled ingests not found or access denied");
  });
  ...
  it("should enforce access control via overrideAccess: false", async () => {
    ...
    expect(mockPayload.findByID).toHaveBeenCalledWith(
      expect.objectContaining({ collection: "scheduled-ingests", id: 1, user: mockUser, overrideAccess: false })
    );
  });
