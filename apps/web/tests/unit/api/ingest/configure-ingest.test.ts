...
/** Route context with params Promise, as expected by apiRoute handlers. */
const routeContext = { params: Promise.resolve({}) };

const createRequest = (body: Record<string, unknown>, user: Record<string, unknown> = mockUser) => {
  // The apiRoute handler authenticates via payload.auth(), so configure that mock per-user
  mocks.mockPayload.auth.mockResolvedValue({ user });
  return {
    user,
...
describe.sequential("POST /api/ingest/configure", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    // Default: auth returns the default user
  ...
  describe("Successful Import", () => {
    it("should create import file and return success response", async () => {
      ...
      expect(typeof body.ingestFileId).toBe("number");
      expect(body.ingestFileId).toBeGreaterThan(0);
  ...
  describe("scheduled ingest Creation", () => {
    ...
    it("should return 429 when scheduled ingest quota is exceeded (Bug 15)", async () => {
    ...
    it("should not create scheduled ingest when createSchedule is not provided", async () => {
      ...
      expect(response.status).toBe(200);
      expect(body.scheduledIngestId).toBeUndefined();

      // Verify scheduled-ingests was NOT called
      const scheduledIngestCalls = mocks.mockPayload.create.mock.calls.filter(
        (call: unknown[]) => (call[0] as Record<string, unknown>).collection === "scheduled-ingests"
      );
      expect(scheduledIngestCalls).toHaveLength(0);
    });
  });
