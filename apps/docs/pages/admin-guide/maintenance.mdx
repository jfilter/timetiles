# Maintenance & Operations

Routine tasks and troubleshooting for TimeTiles administrators.

## Daily Operations

### Health Monitoring

Check system health regularly:

```bash
# API health check
curl https://your-domain.com/api/health

# Container status (production)
docker-compose -f deploy/docker-compose.prod.yml ps

# Database connectivity
docker-compose -f deploy/docker-compose.prod.yml exec postgres pg_isready

# View recent logs
./deploy.sh logs --tail=100
```

### Backup Procedures

#### Quick Commands

```bash
# Database backup (default)
./deploy.sh backup

# Full backup (database + uploads)
./deploy.sh backup full

# Setup automatic daily backups at 2 AM
./deploy.sh backup auto

# List available backups
./deploy.sh backup list

# Clean old backups (>30 days)
./deploy.sh backup clean
```

#### Restore from Backup

```bash
# List available backups
./deploy.sh restore

# Restore database
./deploy.sh restore db-20241220-143022.sql.gz

# Restore database and uploads
./deploy.sh restore db-20241220-143022.sql.gz uploads-20241220-143022.tar.gz
```

The backup system automatically:
- Keeps only the last 30 database backups
- Stores backups in `deploy/backups/`
- Names files with timestamps for easy identification
- Compresses backups to save space

## Updates & Upgrades

### Application Updates

```bash
# 1. Backup first
./deploy.sh backup

# 2. Pull latest code
git pull origin main

# 3. Rebuild and deploy
./deploy.sh down
./deploy.sh build
./deploy.sh up

# 4. Run migrations
./deploy.sh migrate

# 5. Verify health
curl https://your-domain.com/api/health
```

### Dependency Updates

```bash
# Update dependencies (development)
pnpm update --latest
pnpm test
pnpm build

# Commit and deploy
git add -A
git commit -m "chore: update dependencies"
git push
```

### Database Migrations

```bash
# Check pending migrations
docker-compose -f deploy/docker-compose.prod.yml exec web \
  pnpm payload migrate:status

# Run migrations
./deploy.sh migrate
```

## Performance Optimization

### Database Maintenance

Run regularly (weekly recommended):

```bash
# Connect to database
docker-compose -f deploy/docker-compose.prod.yml exec postgres psql -U timetiles_user timetiles

# Vacuum and analyze
VACUUM ANALYZE;

# Reindex for performance
REINDEX DATABASE timetiles;

# Update table statistics
ANALYZE;
```

### Query Performance

```sql
-- Enable query stats
CREATE EXTENSION IF NOT EXISTS pg_stat_statements;

-- Find slow queries
SELECT 
  mean_exec_time,
  calls,
  query 
FROM pg_stat_statements 
ORDER BY mean_exec_time DESC 
LIMIT 10;

-- Check index usage
SELECT 
  schemaname,
  tablename,
  indexname,
  idx_scan
FROM pg_stat_user_indexes
ORDER BY idx_scan;
```

### Redis Cache Management

```bash
# Monitor Redis memory
docker-compose -f deploy/docker-compose.prod.yml exec redis redis-cli INFO memory

# Clear cache if needed
docker-compose -f deploy/docker-compose.prod.yml exec redis redis-cli FLUSHALL

# View cache keys
docker-compose -f deploy/docker-compose.prod.yml exec redis redis-cli KEYS "*"
```

## Monitoring

### Log Management

```bash
# View live logs
./deploy.sh logs -f

# Check specific service
docker-compose -f deploy/docker-compose.prod.yml logs web --tail=100
docker-compose -f deploy/docker-compose.prod.yml logs postgres --tail=50

# Search logs
docker-compose -f deploy/docker-compose.prod.yml logs | grep ERROR
docker-compose -f deploy/docker-compose.prod.yml logs | grep "import.*failed"
```

### Disk Usage

```bash
# Check disk space
df -h

# Find large files
du -h /var/lib/docker/volumes | sort -rh | head -20

# Clean up Docker
docker system prune -a --volumes
```

### Resource Usage

```bash
# Container stats
docker stats

# Database size
docker-compose -f deploy/docker-compose.prod.yml exec postgres \
  psql -U timetiles_user -d timetiles -c \
  "SELECT pg_size_pretty(pg_database_size('timetiles'));"

# Table sizes
docker-compose -f deploy/docker-compose.prod.yml exec postgres \
  psql -U timetiles_user -d timetiles -c \
  "SELECT schemaname, tablename, pg_size_pretty(pg_total_relation_size(schemaname||'.'||tablename)) AS size 
   FROM pg_tables 
   WHERE schemaname = 'payload' 
   ORDER BY pg_total_relation_size(schemaname||'.'||tablename) DESC;"
```

## Troubleshooting

### Common Issues

#### Application Won't Start

```bash
# Check logs
./deploy.sh logs web

# Verify environment
docker-compose -f deploy/docker-compose.prod.yml exec web env | grep -E "DATABASE|PAYLOAD"

# Test database connection
docker-compose -f deploy/docker-compose.prod.yml exec web \
  psql $DATABASE_URL -c "SELECT 1"
```

#### Import Failures

```bash
# Check import job status
docker-compose -f deploy/docker-compose.prod.yml exec web \
  psql $DATABASE_URL -c "SELECT * FROM payload.import_jobs ORDER BY created_at DESC LIMIT 5;"

# View error details
docker-compose -f deploy/docker-compose.prod.yml logs web | grep -A5 "import.*error"

# Reset stuck imports
docker-compose -f deploy/docker-compose.prod.yml exec web \
  psql $DATABASE_URL -c "UPDATE payload.import_jobs SET status = 'failed' WHERE status = 'processing' AND updated_at < NOW() - INTERVAL '1 hour';"
```

#### High Memory Usage

```bash
# Check Node.js memory
docker-compose -f deploy/docker-compose.prod.yml exec web \
  node -e "console.log(process.memoryUsage())"

# Restart to clear memory
docker-compose -f deploy/docker-compose.prod.yml restart web

# Increase memory limit if needed
# Edit deploy/.env.production
NODE_OPTIONS="--max-old-space-size=4096"
```

#### SSL Certificate Issues

```bash
# Check certificate expiry
docker-compose -f deploy/docker-compose.prod.yml exec nginx \
  openssl x509 -in /etc/letsencrypt/live/your-domain.com/cert.pem -text -noout | grep "Not After"

# Force renewal
docker-compose -f deploy/docker-compose.prod.yml exec certbot \
  certbot renew --force-renewal

# Restart nginx
docker-compose -f deploy/docker-compose.prod.yml restart nginx
```

### Debug Mode

Enable detailed logging for troubleshooting:

```bash
# Edit deploy/.env.production
LOG_LEVEL=debug

# Restart application
./deploy.sh restart

# View debug logs
./deploy.sh logs web | grep DEBUG
```

## Security Updates

### SSL Certificate Renewal

Certificates auto-renew, but verify monthly:

```bash
# Check renewal
docker-compose -f deploy/docker-compose.prod.yml exec certbot \
  certbot certificates

# Test renewal
docker-compose -f deploy/docker-compose.prod.yml exec certbot \
  certbot renew --dry-run
```

### Security Patches

```bash
# Update base images
docker-compose -f deploy/docker-compose.prod.yml pull
./deploy.sh down
./deploy.sh up

# Check for vulnerabilities
pnpm audit
```

## Getting Help

- **Documentation**: Review this guide and [deployment docs](./deployment)
- **Issues**: [GitHub Issues](https://github.com/jfilter/timetiles/issues)
- **Community**: [GitHub Discussions](https://github.com/jfilter/timetiles/discussions)
- **Logs**: Always check logs first with `./deploy.sh logs`
