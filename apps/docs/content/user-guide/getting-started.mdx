...
TimeTiles works with CSV, Excel, or ODS files containing events. Your data needs:
...
> For a deeper look at how data ingestion works — formats, schema detection, geocoding, and scheduling — see [Data Ingestion](./data-ingestion).

1. **Access the Import Page**
   - Navigate to the ingest page (usually `/ingest` or via "New Chronicle" button)
...
- **Share publicly** - The URL includes all filter state, so bookmarks and shared links preserve the view
- **Export data** - Download all your data as a ZIP archive from Account Settings
