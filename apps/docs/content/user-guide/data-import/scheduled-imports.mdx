...
Scheduled imports use HTTP caching (RFC 7234) to avoid re-downloading unchanged data. If the source server sends appropriate `Cache-Control` headers, TimeTiles respects them. See [Usage Limits](/admin-guide/usage-limits#http-caching) for cache configuration.
