...
**AI Output:** Results saved to `apps/web/.test-results/`, `apps/web/.lint-results/`, `apps/web/.typecheck-results/` (timestamped JSON files)

### Analyzing Test Results

When tests fail, use `jq` to extract detailed information from the latest JSON results:
...
cat apps/web/.test-results/$(ls -t apps/web/.test-results/ | head -1) | jq '.testResults[] | select(.status=="failed") | .assertionResults[] | select(.status=="failed")'

# Count total failures
cat apps/web/.test-results/$(ls -t apps/web/.test-results/ | head -1) | jq '[.testResults[] | select(.status=="failed")] | length'
```

### Analyzing Lint Results

Extract lint errors and top violations (uses latest result file):
...
cat apps/web/.lint-results/$(ls -t apps/web/.lint-results/ | head -1) | jq '.[] | select(.errorCount > 0) | .messages[] | select(.severity == 2)'

# Full error details with location
cat apps/web/.lint-results/$(ls -t apps/web/.lint-results/ | head -1) | jq '[.[] | select(.errorCount > 0) | {file: .filePath, errors: [.messages[] | select(.severity == 2) | {line, column, message, ruleId}]}]'
```

### Analyzing TypeScript Errors

Extract type errors from typecheck results (uses latest result file):

```bash
# All type errors
cat apps/web/.typecheck-results/$(ls -t apps/web/.typecheck-results/ | head -1) | jq '.errors'

# Errors by file
cat apps/web/.typecheck-results/$(ls -t apps/web/.typecheck-results/ | head -1) | jq '.summary.byFile'

# Errors by error code
cat apps/web/.typecheck-results/$(ls -t apps/web/.typecheck-results/ | head -1) | jq '.summary.byCode'
