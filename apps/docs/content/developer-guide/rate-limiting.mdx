...
const quotaCheck = await quotaService.checkQuota(user, QUOTA_TYPES.FILE_UPLOADS_PER_DAY);
...
const result = rateLimitService.checkMultiWindowRateLimit("webhook:token123", [
...
```typescript
import { getRateLimitService, getClientIdentifier } from "@/lib/services/rate-limit-service";
...
if (!result.allowed) {
  throw new Error(`Rate limit exceeded. Limited by ${result.failedWindow} window.`);
}
```
...
```typescript
// Block for 24 hours
rateLimitService.blockIdentifier("abusive-ip", 24 * 60 * 60 * 1000);
```
...
```typescript
const result = rateLimitService.checkTrustLevelRateLimit(clientId, user, "MY_NEW_ENDPOINT");
```
