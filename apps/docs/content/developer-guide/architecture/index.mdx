...
TimeTiles runs **18 background jobs** via Payload's job queue:

- **Import pipeline** (7 jobs): Dataset detection → duplicates → schema → validation → geocoding → event creation
- **Scheduling** (5 jobs): Schedule manager, URL fetch, stuck imports cleanup, pending retries, approval lock cleanup
- **System maintenance** (6 jobs): Quota reset, cache cleanup, schema maintenance, audit log IP cleanup, data export/cleanup, account deletion
