...
This guide provides recommendations for different stakeholders working with the TimeTiles data ingestion pipeline.

## For Data Providers

These practices help ensure smooth ingestion and high-quality event data.

### Data Structure Consistency

**Keep structure consistent across ingestion runs**:
...
- Enables auto-approval of ingestion runs
...
- Archive old ingest-jobs periodically
...
**Datasets with >1,000 fields may impact performance**:
...
- Smaller ingestion runs are more manageable
...
**Let workflows handle orchestration**:

- Don't queue workflows manually from task handlers
- Let `afterChange` hooks on collections queue workflows
- Each task handler does its work and returns structured output
- The workflow handler sequences tasks and inspects results
...
**Keep task handlers focused**:

- Each handler does one thing well
- Return structured output (data, needsReview, or throw)
- Don't queue the next task from a handler -- the workflow handler does that
...
These best practices will help you run a smooth, efficient, and reliable data ingestion pipeline.
