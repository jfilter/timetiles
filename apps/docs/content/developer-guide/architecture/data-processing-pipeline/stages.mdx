...
The TimeTiles data ingestion pipeline consists of eight sequential stages, each responsible for a specific aspect of data transformation and validation. This document provides detailed conceptual explanations of each stage.

## Stage Overview

| Stage                    | Job Handler                 | Purpose                                        | Batch Size       |
| ------------------------ | --------------------------- | ---------------------------------------------- | ---------------- |
| 1. Dataset Detection     | `dataset-detection-job`     | Parse file structure and create ingestion jobs | N/A              |
...
Each ingestion job progresses through all eight stages independently.
...
All created ingestion jobs automatically begin at Stage 2 (Analyze Duplicates).
...
Pause ingestion for human review of schema changes that require approval due to breaking changes or policy requirements. This stage ensures data governance and prevents unintended schema modifications.
...
The ingestion job stores approval information:
...
1. Ingestion reaches await-approval stage
2. Admin receives notification
3. Admin reviews changes in admin interface
4. Admin approves or rejects
5. Approval service updates ingestion job
...
- **If rejected**: Ingestion marked as failed (terminal state)
...
None - this is the final stage. Ingestion transitions to "completed" status.
...
These eight stages work together to ingest uploaded files into structured, validated, and enriched events:
...
Each stage builds on the previous stages' outputs, creating a robust and maintainable data ingestion pipeline.
