...
TimeTiles includes an automatic error recovery system that intelligently retries failed ingestion jobs based on error classification and recovery strategies.

## Overview

The `ErrorRecoveryService` automatically handles ingestion failures through:
...
Ingestion failures are classified into three categories to determine the appropriate recovery strategy.
...
- **Quota Limits**: User exceeded daily ingestion quota (retries after quota reset)
...
```json
{
  ...
  "message": "Ingestion job retry scheduled successfully",
  "retryScheduled": true,
...
```json
{
  ...
  "error": "User has exceeded their daily ingestion quota. Retry will be attempted after quota resets.",
  "statusCode": 403
}
```
...
```json
{ "success": true, "message": "Ingestion job reset successfully", "stage": "ANALYZE_DUPLICATES" }
```
...
**Purpose**: Get actionable recommendations for all failed ingestion jobs
  ...
  "recommendations": [
    {
      ...
      "errorClassification": { "type": "recoverable", "retryable": true, "reason": "Rate limit exceeded (429)" },
    ...
    },
    {
      ...
      "errorClassification": { "type": "permanent", "retryable": false, "reason": "File not found (ENOENT)" },
...
- Monitoring dashboard to show failed ingestion jobs
- Batch processing recovery for multiple failures
- Generating user notifications about failed ingestion runs
...
## Integration with Scheduled Ingestion

For scheduled ingestion (automated URL-based data fetching), error recovery is handled automatically:

1. **Ingestion Fails**: Scheduled ingestion encounters error during processing
...
4. **Max Retries**: After 3 failed attempts, ingestion is marked as failed
5. **Next Scheduled Run**: Next scheduled run will attempt ingestion again from scratch

**Important**: Retry attempts are separate from scheduled runs. A scheduled ingestion that fails will:
