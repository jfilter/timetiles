...
The TimeTiles data ingestion pipeline is a hook-driven, event-based system that transforms uploaded files into structured events. The pipeline automatically orchestrates a series of processing stages through Payload CMS collection hooks and background jobs.
...
The pipeline ingests data through eight sequential stages, from file upload to final event creation:
...
## Eight Ingestion Stages

1. **Dataset Detection** - Parse file structure and create ingestion jobs (one per sheet for Excel files)
...
**Multi-Sheet Support**: Excel files with multiple sheets create separate ingestion jobs that process independently.
