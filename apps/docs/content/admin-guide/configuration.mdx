...
Configure providers directly in the admin interface at `/dashboard/collections/geocoding-providers`:
...
Configure geocoding behavior at `/dashboard/globals/settings`:
...
# Subfolders are created automatically: /media and /ingest-files
UPLOAD_DIR=uploads
...
Check geocoding provider configuration in the admin panel at `/dashboard/collections/geocoding-providers`.
...
| ------------------------------ | ------- | ------------------------------------------------------------------ |
| Allow Private Ingestion        | On      | Users can create private ingestion runs visible only to themselves |
| Enable Scheduled Ingestion     | On      | Users can create automated URL-based ingestion schedules           |
| Enable Public Registration     | On      | New users can self-register accounts                               |
| Enable Event Creation          | On      | New events can be created via ingestion or API                     |
| Enable Dataset Creation        | On      | Users can create new datasets                                      |
| Enable Ingestion Creation      | On      | Users can create new ingestion jobs                                |
| Enable Scheduled Job Execution | On      | Scheduled ingestion runs automatically                             |
| Enable URL Fetch Caching       | On      | Cache URL fetches to reduce external requests                      |

## Scheduled Ingestion

Users can create automated ingestion schedules that fetch data from a URL on a cron schedule.

### Configuration

- Enable/disable via the **Enable Scheduled Ingestion** feature flag
- Users create schedules at `/dashboard/collections/scheduled-ingests`
- Each schedule specifies: URL, cron expression, target catalog/dataset, authentication (optional)

### Authentication Options

Scheduled ingestion supports several authentication methods for fetching protected URLs:

- **None**: Public URLs with no authentication
...
Scheduled ingestion uses HTTP caching (RFC 7234) to avoid re-downloading unchanged data. See [Usage Limits](./usage-limits#http-caching) for cache configuration.

### Monitoring

- View all schedules: `/dashboard/collections/scheduled-ingests`
- Check schedule health: `GET /api/admin/schedule-service`
- System automatically cleans up stuck ingestion jobs via background job

## Audit Log

TimeTiles tracks user and admin actions for accountability and debugging.

### What's Tracked

20 action types across 5 domains:

- **Account**: Login, logout, registration, email change, password change, deletion request/cancel/execute
- **Admin**: User role change, trust level change, feature flag change
- **Data**: Catalog/dataset/event create/update/delete
- **Import**: Import start, import complete, import fail
- **System**: Settings change, geocoding provider change

### Privacy

- IP addresses are stored for 30 days, then cleared by the `audit-log-ip-cleanup` background job
- `performedBy` is only set when the acting user differs from the target user

### Viewing Logs

- Admin panel: `/dashboard/collections/audit-log`
- Filter by user, action type, domain, or date range
