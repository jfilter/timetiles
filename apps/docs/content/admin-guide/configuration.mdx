...
Users can create automated imports that fetch data from a URL on a cron schedule.

### Configuration

- Enable/disable via the **Enable Scheduled Imports** feature flag
...
Scheduled imports support several authentication methods for fetching protected URLs:
...
Scheduled imports use HTTP caching (RFC 7234) to avoid re-downloading unchanged data. See [Usage Limits](./usage-limits#http-caching) for cache configuration.

### Monitoring

- View all schedules: `/dashboard/collections/scheduled-imports`
- Check schedule health: `GET /api/admin/schedule-service`
- System automatically cleans up stuck imports via background job
