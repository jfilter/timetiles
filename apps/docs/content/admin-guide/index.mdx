---
asIndexPage: true
---

# Admin Guide

Documentation for deploying and operating TimeTiles.

## Overview

This guide covers everything administrators need to know:

- Setting up TimeTiles for development or production
- Configuring the application, feature flags, and geocoding providers
- Managing content (pages, branding, navigation), scheduled imports, and audit logs
- Deploying with Docker and managing SSL certificates
- Maintaining and monitoring your instance

## Documentation Structure

### [Installation](./installation)

Prerequisites, system requirements, and initial setup for both development and production environments.

### [Configuration](./configuration)

Complete reference for environment variables, feature flags, content management, scheduled imports, audit logs, and application settings.

### [Production Deployment](./deployment)

Step-by-step guide for deploying TimeTiles in production with Docker, nginx, and SSL/TLS.

### [Maintenance & Operations](./maintenance)

Routine tasks, backups, updates, monitoring, and troubleshooting.

## Quick Start

- **Development**: See [Quick Start Guide](/quick-start) for local setup
- **Production**: See [Deployment Guide](./deployment) for production deployment

## Architecture

TimeTiles consists of:

- **Next.js Application**: Main web application with Payload CMS
- **PostgreSQL + PostGIS**: Spatial database for events and timelines
- **Nginx**: Reverse proxy with SSL termination (production)

## Getting Help

- **Issues**: [GitHub Issues](https://github.com/jfilter/timetiles/issues)
- **Discussions**: [GitHub Discussions](https://github.com/jfilter/timetiles/discussions)
- **Documentation**: You're reading it!

## Related Guides

- [User Guide](/user-guide/getting-started) - For end users
- [Developer Guide](/developer-guide/) - For contributors
- [API Reference](/reference/api) - API documentation
