...
docker-compose -f deployment/docker-compose.prod.yml logs | grep "ingest.*failed"
```
...
docker-compose -f deployment/docker-compose.prod.yml exec web \
  psql $DATABASE_URL -c "SELECT * FROM payload.ingest_jobs ORDER BY created_at DESC LIMIT 5;"

# View error details
docker-compose -f deployment/docker-compose.prod.yml logs web | grep -A5 "ingest.*error"

# Reset stuck ingests
docker-compose -f deployment/docker-compose.prod.yml exec web \
  psql $DATABASE_URL -c "UPDATE payload.ingest_jobs SET status = 'failed' WHERE status = 'processing' AND updated_at < NOW() - INTERVAL '1 hour';"
```
...
- **Logs**: Always check logs first with `timetiles logs`
