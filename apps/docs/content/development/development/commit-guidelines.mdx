...
- **build**: Changes affecting build system or external dependencies
  - Example: `build(deps): upgrade to Next.js 16`
...
```
refactor(api): change ingest endpoint response format

- Changed /api/ingest/preview-schema/upload response structure
- Now returns complete job object instead of just ingest ID
...
```
fix(ingest): update ingest-jobs.ts and add validation
...
4. **Missing scope when files suggest one**:
   - ❌ `fix: update ingest validation` (when changing apps/web/app/api/ingest/)
   - ✅ `fix(ingest): update validation`
