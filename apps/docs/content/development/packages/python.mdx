...
| ---------------------------- | ----------------------------------------------------------------------------------------- |
| `output.write_row(row)`      | Append a dict as a CSV row. Headers are the union of all rows' keys, in first-seen order. |
...
| `TIMESCRAPE_OUTPUT_FILE` | `data.csv` | Output filename. Set automatically from the manifest's `output:` field.        |
