...
git push origin tag ui-v0.2.0
```

The matching workflow triggers automatically on the tag push. Push only the intended release tag; `--tags` also publishes unrelated local tags and can trigger other release workflows.
...
**Build fails** -- Run `pnpm exec tsup` locally in the package directory to debug. From the repository root, check the UI package with `make check-ai PACKAGE=ui` and the scraper SDK with `pnpm --filter @timetiles/scraper typecheck`. `make check-ai PACKAGE=scraper` is an alias for the TimeScrape runner, not the SDK.
