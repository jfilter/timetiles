---
title: Scrapers
description: Run user-defined scripts to extract event data from websites and APIs, then feed results into the TimeTiles ingestion pipeline.
asIndexPage: true
---
...
After a successful run, check the scraper's run history in the dashboard or the **Scrapers** tab at `/account/imports?tab=scrapers`. If `autoImport` is enabled and a `targetDataset` is configured, the CSV flows through the standard ingestion pipeline and events appear on the map.

### Management UI

Users manage scrapers in the **Scrapers** tab at `/account/imports?tab=scrapers`. The old `/account/scrapers` URL redirects there. The page shows:

- A table of scrapers with repository, last run status, runtime, schedule, and last run time
- Repository synchronization errors, including repositories with no scrapers
- Actions to sync a repository, trigger a run, or delete a repository
- Expandable run history with status, duration, errors, and links to dashboard records for full details

Manual repository synchronization refreshes the lists after the background sync finishes. Run stdout/stderr logs are available in the dashboard records, not in an inline log viewer.
...
| Method                                | Description                                                                   |
| ------------------------------------- | ----------------------------------------------------------------------------- |
| `output.write_row(row: dict)`         | Append one row. Headers are the union of all rows' keys, in first-seen order. |
...
| Method                   | Description                                                                   |
| ------------------------ | ----------------------------------------------------------------------------- |
| `output.writeRow(row)`   | Append one row. Headers are the union of all rows' keys, in first-seen order. |
...
The `schedule-manager` job (which also handles scheduled URL imports) checks enabled scrapers every minute. When a scraper is due, it queues the `scraper-ingest` workflow on the `ingest` queue. The workflow runs the `scraper-execution` task and, when auto-import produces an ingest file, detects its sheets and runs the import pipeline.
...
2. A unique webhook URL is generated automatically. Copy it immediately: the plaintext token is returned only when created or rotated; only its hash is stored.
...
When a scraper has `autoImport` enabled, successful runs with a downloadable CSV containing at least one data row feed into the TimeTiles import pipeline. A `targetDataset` is optional.

### How It Works

1. Scraper runs and produces CSV output.
2. The `scraper-execution` task downloads the CSV from the runner's output URL, using the configured runner API key when required.
...
The `scraper-ingest` workflow processes the resulting file through the shared import pipeline.

### Configuration

Set these fields on the scraper record (via the manifest or the admin UI):

- **targetDataset**: Optional existing dataset to import events into. When omitted, the import pipeline uses its normal dataset detection.
- **autoImport**: Must be `true` to enable the pipeline.

When `autoImport` is disabled, no ingest file is created automatically. Successful zero-row runs are recorded as successful but skip the import pipeline.
...
**Response**: `200 OK` with `{ "message": "Scraper run queued" }`.
...
| `GET`  | `/health`          | None   | Health check. `{"status": "ok"}`, or 503 if images are missing. |
...
- [Usage Limits](/self-hosting/usage-limits) -- Full quota system documentation
