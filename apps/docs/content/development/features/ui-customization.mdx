...
The frontend layout (`app/[locale]/(frontend)/layout.tsx`) declares a single `next/script` component with `id="theme-preset"` and `strategy="beforeInteractive"`. It reads the stored preset and applies its class to both `<html>` and `<body>`. Both elements need the class because `next/font` defines font variables on `<body>`.

Keep initialization in that layout script aligned with `useThemePreset()`; do not add separate head/body scripts.
