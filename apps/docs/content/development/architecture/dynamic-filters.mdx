...
{
  groups: [
    { id: "basic", labels: { en: "Basic Information" }, order: 1, filters: ["name", "status", "category"] },
    {
      id: "location",
      labels: { en: "Location" },
      order: 2,
    ...
    { id: "temporal", labels: { en: "Time & Date" }, order: 3, filters: ["createdAt", "eventDate", "year"] },
  ];
}
```
  ...
  dependencies: [
    {
      ...
      mapping: { USA: ["New York", "Los Angeles", "Chicago"], UK: ["London", "Manchester", "Birmingham"] },
    },
  ];
}
```
...
{
  computedFilters: [
    { id: "fullName", label: "Full Name", fields: ["firstName", "lastName"], operator: "concat", searchable: true },
  ];
}
```
