...
- `ingest-files` - File uploads
- `ingest-jobs` - Import processing
- `events` - Event creation
- `scheduled-ingests` - Active schedules

**Background Jobs:**

- `quota-reset-job` - Daily quota counter reset (midnight UTC)
- `cache-cleanup` - Cache cleanup (every 6 hours)

## Related Documentation

- [Usage Limits Configuration](/self-hosting/usage-limits) - Quotas, rate limits, and cache settings
