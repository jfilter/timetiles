...
- **Memory efficiency**: Batch processing limits the rows buffered for each processing step
- **Database efficiency**: Minimal database writes during processing
- **Clean separation**: Processing logic is independent of storage
- **Recovery support**: Workflows retry failed tasks and resume reviewed sheets at defined processing stages
...
| `scheduled-ingest` | `lib/ingest/trigger-service.ts`              | URL fetch + full pipeline            |
| `scraper-ingest`   | `schedule-manager` job                       | Scraper execution + full pipeline    |
| `ingest-process`   | Review approval hook or retry/reset endpoint | Resume or retry a sheet's processing |

### Workflow Queueing

Collection `afterChange` hooks own upload and review-approval triggers:
...
Other entry points queue workflows directly: `lib/ingest/trigger-service.ts` queues scheduled URL imports, the schedule manager queues scraper runs, and the ingest-job retry/reset endpoints queue `ingest-process`. Reuse the entry point that owns the action rather than also queueing a second workflow. Hooks alone do not guarantee exactly-once execution.

### Concurrency Control

Each workflow defines a resource-specific `concurrency` key in `apps/web/lib/jobs/workflows/`. These keys coordinate concurrent execution; they do not replace the entry points' queueing guards or make repeated requests exactly-once. Consult the workflow definitions for the current key formats.
...
- **No Double-Queueing**: Respect the existing trigger owner, whether a hook, service, scheduled handler, or API endpoint
...
Different stages use different batch sizes optimized for their specific needs. See [Processing Stages](./stages) for batch sizes per stage.

- **Duplicate analysis** — reads the file in batches while building an in-memory duplicate map
...
- Processing files without loading all source rows into memory at once
- Incremental progress tracking
- Partial recovery from failures

Batch progress is not a durable resume cursor. Review approval and manual recovery select a workflow stage; a restarted duplicate-analysis task reads the file again from the beginning.

Batching does not bound all memory usage. Duplicate analysis retains a map of unique IDs and a list of duplicate rows across batches. The unique-row limit stops oversized sheets, but batch size alone is not a guarantee against memory exhaustion.
...
- Batch processing limits per-batch row buffers
- Configurable batch sizes tune buffer usage, not the accumulated duplicate-analysis state
