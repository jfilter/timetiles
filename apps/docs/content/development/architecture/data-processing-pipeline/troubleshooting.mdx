...
This guide helps you diagnose and resolve common issues with the TimeTiles data ingestion pipeline.
...
- Ingestion status shows "processing" for extended period
...
### Schema Review Required (NEEDS_REVIEW)

**Symptoms**:

- Ingestion job shows `NEEDS_REVIEW` status
- Schema drift detected in automated import
- Admin interface shows pending reviews

**Possible Causes**:

1. **Breaking schema changes detected** in automated import (scheduled or scraper)
2. **Dataset schema is locked**
3. **Auto-approval disabled**
4. **High duplicate rate or geocoding failure rate**
...
2. **Limit Concurrency**: Reduce max concurrent ingestion runs
...
- Ingestion taking much longer than before
...
- Ingestion completes but with errors
...
1. Query ingest-jobs collection for detailed state
2. Check ingest-files for overall status
...
TimeTiles uses Payload's built-in workflow retry system for error recovery. See the [Error Recovery](./error-recovery) documentation for complete details on:

- Error model (throw for transient failures, needsReview for human review)
- Automatic retry with cached completed tasks
- Per-sheet error isolation in multi-sheet workflows
- Recovery API endpoints (`/retry`, `/reset`)
...
- Set up alerts for stuck ingestion jobs (>1 hour in same stage)
...
- Validate data before ingestion when possible
...
These troubleshooting techniques and recovery procedures should help you diagnose and resolve most common ingestion pipeline issues effectively.
