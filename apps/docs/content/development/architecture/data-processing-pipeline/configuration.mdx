...
Background job workers run as separate processes, one per queue. See [Production Workers](../background-jobs#production-workers) for the Payload commands, polling intervals, and scheduling flags used in production. Retry behavior is handled by Payload's built-in workflow system.
