...
Query parameters remain part of the key: `/data?b=2&a=1` matches `/data?a=1&b=2`, but not `/data`. Keys also include the HTTP method, user ID (or an anonymous marker), and an authentication fingerprint when supplied by the caller.
