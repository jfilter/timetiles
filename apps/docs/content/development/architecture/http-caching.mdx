...
- The `cache-cleanup` maintenance job runs every 6 hours, but only cleans the cache directory visible to its process. In the production Compose stack, the ingest and maintenance containers have separate `/tmp` directories, so this job does not clean the ingest container's default cache. Size-based eviction still runs when entries are written.
