...
The TimeTiles REST API provides programmatic access to event data, geospatial clustering, temporal histograms, and ingest progress tracking. Event read endpoints return JSON and share filters for catalog, dataset, date range, and geographic bounds; other routes have their own contracts.
...
| `/api/ingest/*`               | Ingestion schema preview, configure, validate  |
| `/api/ingest-jobs/*`          | Ingestion job management (reset, retry)        |
| `/api/users/*`                | Account management (password, email, deletion) |
| `/api/account/*`              | Account deletion summary                       |
| `/api/admin/*`                | Admin operations (schedule, jobs)              |
| `/api/quotas`                 | User quota status                              |
| `/api/feature-flags`          | Feature flag state                             |
| `/api/data-exports/*`         | Data export request and download               |
| `/api/scheduled-ingests/*`    | Manual trigger for scheduled ingestion         |
...
Payload CMS auto-generates REST endpoints for all collections (e.g., `/api/users`, `/api/events`). These are used by the admin panel and enforce collection-level access control; whether authentication is required depends on the collection and operation.
...
1. **Login** through the `/dashboard/login` interface or Payload's authentication endpoints
...
Public event reads allow anonymous access to public data. A session enables access to additional data permitted by the caller's account; it does not bypass collection or route access controls.
...
| ----------- | ----------- | ----------------------------------- | ---------------------------------------------------------------- |
| `catalog`   | integer     | Filter by numeric catalog ID        | `catalog=1`                                                      |
| `datasets`  | string      | Comma-separated numeric dataset IDs | `datasets=2,3`                                                   |
...
Categorical (`ff`) and numeric range (`rf`) filters are JSON-encoded query parameters; see [Dynamic Filters](/development/features/dynamic-filters) for their wire formats. The shared contract is `EventFiltersSchema` in `apps/web/lib/schemas/events.ts`.
...
See [Usage Limits](/self-hosting/usage-limits) for details on rate limiting and quotas.
...
| ----------- | -------- | -------- | ---------------------------------------------------- |
| `catalog`   | integer  | No       | Filter by numeric catalog ID                         |
| `datasets`  | string   | No       | Comma-separated numeric dataset IDs                  |
  ...
  features: [
    {
      ...
      properties: {
        ...
        count?: number,              // Events represented by the feature
        title?: string,              // For individual events
        eventId?: number,            // When the location represents one event
        sourceCells?: string[],      // H3 cells represented by a cluster
        h3Cell?: string
      }
    }
  ]
...
The default algorithm is H3; `clusterAlgorithm` also accepts `grid-k` and `dbscan`. Resolution depends on zoom and the clustering parameters, not a fixed zoom cutoff. At the finest H3 resolution, co-located events remain grouped as `event-location` features.

See `MapClustersQuerySchema` and `ClusterFeatureSchema` in `apps/web/lib/schemas/events.ts` for the full parameter and response contracts.
...
| `startDate`     | ISO 8601 | No       | Filter events after this date                              |
| `endDate`       | ISO 8601 | No       | Filter events before this date                             |
| `targetBuckets` | integer  | No       | Target number of histogram buckets (default: 30, max: 500) |
| `minBuckets`    | integer  | No       | Minimum number of buckets (default: 20, max: 500)          |
| `maxBuckets`    | integer  | No       | Maximum number of buckets (default: 50, max: 500)          |
...
```typescript
{
  ...
  metadata: {
    ...
    topDatasets: Array<{ id: number, name: string, count: number }>,
    topCatalogs: Array<{ id: number, name: string, count: number }>
  }
}
```
...
The PostgreSQL function `calculate_event_histogram` automatically determines optimal bucket sizes based on the data range and the `targetBuckets`/`minBuckets`/`maxBuckets` parameters. The actual bucket size used is returned in `metadata.bucketSizeSeconds`.

If `minBuckets` exceeds `maxBuckets`, the route clamps it to `maxBuckets`. Source counts represent distinct datasets and catalogs with matching timestamped events. `topDatasets` and `topCatalogs` contain up to five sources ranked by event count (ties use ascending ID), under the same access controls and filters as the histogram.
...
Retrieve real-time progress information for a data ingestion operation.

```http
GET /api/ingest/:ingestId/progress
```
...
| Parameter  | Type   | Required | Description    |
| ---------- | ------ | -------- | -------------- |
| `ingestId` | string | **Yes**  | Ingest file ID |
  ...
  jobs: [
    {
      ...
      errors: number,                     // Count of retained error details, not all row failures
...
1. `analyze-duplicates` - Finding duplicate rows
2. `detect-schema` - Analyzing file structure and field types
3. `validate-schema` - Comparing detected schema with existing dataset schema
4. `needs-review` - Paused for user review in manual or automated imports; `reviewReason` and `reviewDetails` identify the data-quality or configuration issue
5. `create-schema-version` - Persisting approved schema
6. `geocode-batch` - Geocoding location fields
7. `create-events` - Creating event records in database
8. `completed` - Ingestion finished successfully
9. `failed` - Ingestion failed with errors

**Example:**

```bash
curl "https://your-instance.com/api/ingest/123/progress"
```

**Implementation:** `apps/web/app/api/ingest/[ingestId]/progress/route.ts`
...
```typescript
{
  quotas: {
    ...
    urlFetchesPerDay: {...},    // same shape
    importJobsPerDay: {...},    // same shape
    activeSchedules: {...},     // same shape
    totalEvents: {...},         // same shape
    eventsPerImport: {...},     // same shape
    maxFileSizeMB: {
      limit: number
    }
  }
...
**Implementation:** `apps/web/app/api/quotas/route.ts`
...
The API uses URL fetch caching for scheduled imports. See [HTTP Caching](/development/architecture/http-caching) for details.
...
The API routes have comprehensive TypeScript documentation in the source code.
...
const response = await fetch(
  "https://your-instance.com/api/v1/events?" +
    new URLSearchParams({
      bounds: JSON.stringify({ north: 37.8, south: 37.7, east: -122.3, west: -122.5 }),
      startDate: "2024-01-01",
      endDate: "2024-12-31",
      limit: "100",
    })
...
```typescript
async function pollIngestProgress(ingestId: string) {
  const response = await fetch(`https://your-instance.com/api/ingest/${ingestId}/progress`);
...
const response = await fetch(
  "https://your-instance.com/api/v1/events/temporal?" +
    new URLSearchParams({ catalog: "1", startDate: "2024-01-01", endDate: "2024-12-31" })
);
...
- [Usage Limits](/self-hosting/usage-limits) - Rate limiting, quotas, and trust levels
- [HTTP Caching](/development/architecture/http-caching) - Caching for scheduled URL imports
