...
The TimeTiles REST API provides programmatic access to event data, geospatial clustering, temporal histograms, and ingest progress tracking. All endpoints return JSON responses and support filtering by catalog, dataset, date range, and geographic bounds.
...
const response = await fetch(
  "https://your-instance.com/api/v1/events/temporal?" +
    new URLSearchParams({ catalog: "my-catalog", startDate: "2024-01-01", endDate: "2024-12-31" })
);
