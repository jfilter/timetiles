...
Validation includes staged-file context. If no files are staged, the configuration falls back to the latest commit's changed files. The separate `scripts/validate-commit.ts` helper is not the hook's source of truth.
