...
hooks: {
  afterChange: [
    async ({ doc, req }) => {
      ...
      await req.payload.update({ collection: "users", id: doc.userId, data: { lastUpdated: new Date() } });
    },
  ];
}
```
...
- [ ] **Use `context` flags** to prevent infinite hook recursion
  - Common flags: `skipUsageHooks`, `skipQuotaChecks`, `skipIngestFileHooks`
...
See `apps/web/lib/collections/scheduled-ingests/index.ts` for a complete example of:
...
See `apps/web/lib/collections/ingest-files/hooks.ts` for:
