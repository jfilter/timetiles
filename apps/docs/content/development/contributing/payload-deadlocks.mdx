...
Pass the original request rather than a copy. Preserving a transaction ID on a
copied object cannot preserve a database transaction that Payload has rolled back.
Collection hooks must also propagate failures from audit-related owner lookups,
not catch the entire audit path as best-effort.
