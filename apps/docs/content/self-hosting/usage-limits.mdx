...
Trust levels, quotas, rate limiting, and HTTP caching configuration. All values shown are the defaults — customize them in `config/timetiles.yml` (see [Configuration](./configuration#config-file-timetilesyml)).
...
Can also be set via environment variables (`URL_FETCH_CACHE_DIR`, `URL_FETCH_CACHE_MAX_SIZE`, etc.).
