...
Trust levels, quotas, rate limiting, and HTTP caching configuration. Unless marked as a fixed technical limit, values shown are defaults — customize them in `config/timetiles.yml` (see [Configuration](./configuration#config-file-timetilesyml)).
...
| 5     | Unlimited  | Most quotas unlimited (admins)  |
