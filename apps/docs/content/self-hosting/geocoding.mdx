...
- Expired entries are removed when looked up again. There is currently no scheduled bulk cleanup for the location cache; the six-hour cache job only cleans the URL-fetch cache
