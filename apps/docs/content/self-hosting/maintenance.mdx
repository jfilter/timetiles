...
Each snapshot holds either the database or the uploads. A snapshot ID restores that one half; `latest`
restores the newest `db` snapshot and the newest `uploads` snapshot together.
...
`latest` refuses to run unless the repository holds at least one snapshot of each tag, and fails unless both halves
were restored. To recover an older state, restore the chosen `db` and `uploads` snapshot IDs one after the other. The
command reports what it restored, for example `Restore complete! Restored: database, uploads`.
