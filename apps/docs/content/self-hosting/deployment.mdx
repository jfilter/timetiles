...
The command asks for a password twice and creates a verified admin account. It
refuses if an account with that address already exists, so it cannot be used to
change an existing user's password or role.

Self-registration through the web interface always creates a regular user, never
an admin — this command is the only way to create one.
