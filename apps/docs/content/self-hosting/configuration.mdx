...
Optional — only needed if running the [TimeScrape runner](/reference/scrapers).
...
Batch sizes can be set via environment variables **or** in the config file. Environment variables take precedence over the config file.

```bash
BATCH_SIZE_EVENT_CREATION=1000
BATCH_SIZE_SCHEMA_DETECTION=10000
BATCH_SIZE_DUPLICATE_ANALYSIS=5000
BATCH_SIZE_DATABASE_CHUNK=1000
```
...
In Docker, the file is mounted read-only into all containers via `CONFIG_FILE` in your `.env.production`:

```bash
CONFIG_FILE=./config/timetiles.yml
```
...
Tune the import pipeline's processing batch sizes. Can also be overridden via `BATCH_SIZE_*` environment variables (env vars take precedence).
