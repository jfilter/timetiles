...
The runner spawns containers from two pre-built base images. Build them from the `apps/scraper/images/` directory:
...
Create an `.env` file in `apps/scraper/`:
...
- Fed into the import pipeline, which creates events in the database.
...
| Base images                          | Built from Dockerfiles in `apps/scraper/images/` | Rebuild from source, or push to a container registry |
...
1. Update the Dockerfile in `apps/scraper/images/python/` or `apps/scraper/images/node/`.
...
- [Production Deployment](/admin-guide/deployment) -- Main TimeTiles deployment guide
