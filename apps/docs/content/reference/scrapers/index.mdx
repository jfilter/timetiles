...
After a successful run, check the scraper's run history in the dashboard or the management UI at **/account/scrapers**. If `autoImport` is enabled and a `targetDataset` is configured, the CSV flows through the standard import pipeline and events appear on the map.
