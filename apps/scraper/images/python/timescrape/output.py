"""CSV output helper for TimeScrape scrapers.
...
class OutputWriter:
    ...
    def __init__(self):
        ...
        self._output_dir = os.environ.get("TIMESCRAPE_OUTPUT_DIR", "/output")
    ...
    def save(self, filename: str | None = None) -> str:
        ...
        Args:
            filename: Override the default output filename.
        ...
        if not self._rows:
            # Write empty file with headers if available
        ...
        # Use headers from first row if not explicitly set
        if self._headers is None:
