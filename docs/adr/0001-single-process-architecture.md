...
TimeTiles uses several in-memory data structures for performance-critical operations: rate limiting, schedule management, view caching, geocoding throttling, and stage transition locking. These structures live in module-level variables or class-level static fields within a single Node.js process.
...
|-----------|------|-------|-------------------------------|
| **RateLimitService** | `lib/services/rate-limit-service.ts` | `Map<string, RateLimitEntry>` + cleanup interval | Rate limits split across processes — users can exceed limits by hitting different processes |
...
3. **RateLimitService** — Replace in-memory `Map` with Redis counters (e.g., `INCR` + `EXPIRE`)
